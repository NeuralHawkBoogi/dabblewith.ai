...
function run() {
  ...
  assert(approved.version === 2, `Expected version 2 after approve, got ${approved.version}`);
  ...
  console.log('Step 3 PASS: owner_a approved, activatedAt stamped, version bumped to 2');
  ...
  console.log('Step 4 PASS: owner_b rejected with reason, version bumped to 2');

  // 5. List pending — expect 0
  ...
  console.log('Step 5 PASS: listPending returns 0 after all reviewed');

  // 6. Verify persisted state matches in-memory returns
  ...
  console.log('Step 6 PASS: persisted sessions match stamped values');

  // 7. advance() on activated/rejected sessions reports done
  ...
  console.log('Step 7 PASS: advance() on terminal admin states reports done');

  // 8. Guard: approve/reject on non-pending session should throw
  let threw = false;
  try { ag.approve(storageDir, 'owner_a', 'community_a', 'x'); } catch { threw = true; }
  assert(threw, 'approve on non-pending session should throw');
  console.log('Step 8 PASS: double-approve guard throws');
